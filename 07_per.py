...
class ReplayBuffer:
    ...
    def store(self, obs: np.ndarray, act: np.ndarray, rew: float, next_obs: np.ndarray, done: bool):
        ...
        self.ptr = (self.ptr + 1) % self.max_size  # modulo prevents ptr from overflowing
...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def sample_batch(self, beta: float = 0.4) -> Dict[str, np.ndarray]:
        ...
        return dict(
            obs=obs, next_obs=next_obs, acts=acts, rews=rews, done=done, weights=weights, indices=indices
        )
...
class Network(nn.Module):
    def __init__(self, in_dim: int, out_dim: int):
        ...
        self.layers = nn.Sequential(
            nn.Linear(in_dim, 128), nn.ReLU(), nn.Linear(128, 128), nn.ReLU(), nn.Linear(128, out_dim)
        )
